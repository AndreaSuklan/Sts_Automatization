...
# Pre-load backgrounds globally so all CPU processes can access them
...
def generate_single_image(i):
    """Worker function to generate a single image and label."""
    if not bg_files:
        return
    ...
    for j, (card_name, card_path) in enumerate(hand):
        ...
        for x, y in corners:
            ...
            # Calculate normalized coordinates for cropped image
        ...
        # 3. Write the instance segmentation format to .txt
    ...
    # Reduced print frequency so multiple processes don't spam the console
    if i % 1000 == 0 and i > 0:
...
def generate_dataset():
    ...
    print(f"Starting generation of {NUM_IMAGES_TO_GENERATE} images using all CPU cores...")
    
    # Use ProcessPoolExecutor to distribute the work
    with ProcessPoolExecutor() as executor:
        executor.map(generate_single_image, range(NUM_IMAGES_TO_GENERATE))
